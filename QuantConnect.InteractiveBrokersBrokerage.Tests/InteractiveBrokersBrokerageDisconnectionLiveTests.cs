...
namespace QuantConnect.Tests.Brokerages.InteractiveBrokers
{
    ...
    public class InteractiveBrokersBrokerageDisconnectionLiveTests
    {
        ...
        public void ReportsTheDisconnectionWhenTheGatewayDiesAndDoesNotComeBack()
        {
            ...
            // the heart beat deliberately stays quiet while a restart or the IB server reset times account for
            // being down, and this test is about the case where nothing accounts for it. Running inside one of
            // those windows would prove nothing, so skip instead. Note the check is made once: starting the
            // test within ~15 minutes of the daily 23:00 local cut off can still drift into it.
            if (IsWithinExpectedDisconnectionWindow(brokerage, out var reason))
            {
        ...
        private static bool IsWithinExpectedDisconnectionWindow(InteractiveBrokersBrokerage brokerage, out string reason)
        {
            var arguments = new object[] { null };
            var result = (bool)typeof(InteractiveBrokersBrokerage)
                .GetMethod("IsWithinExpectedDisconnectionWindow", BindingFlags.NonPublic | BindingFlags.Instance)
                .Invoke(brokerage, arguments);
